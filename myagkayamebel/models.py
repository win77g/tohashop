...
# модеь продукта
class MyagkayamebelModel(models.Model):
    name = models.CharField(verbose_name='Название',max_length=120,blank=True, null=True ,unique=True)
...
def pre_save_product_slug(sender,instance, *args, **kwargs):
    if not instance.slug:
        slug = slugify(translit(instance.name, reversed=True))
        instance.slug = slug
pre_save.connect(pre_save_product_slug, sender=MyagkayamebelModel)
# -----------------------------end product-------------------------------------------------------------
#  --------------------------Gallery-------------------------------------------------------
# создание названия фотки
def image_gallary_folder(instance,filename):
    ...
    return "{0}/{1}".format(instance.slug,filename)
# фотки продукта
class MyagkayamebelImage(models.Model):
    product = models.ForeignKey(MyagkayamebelModel,blank=True, null=True, default=None,on_delete=models.CASCADE)
...
def pre_save_imagegallery_slug(sender,instance, *args, **kwargs):
    ...
    if not instance.slug:
        ...
        instance.slug = slug
pre_save.connect(pre_save_imagegallery_slug, sender=MyagkayamebelImage)
# ---------------------------------------end gallery----------------------------------
