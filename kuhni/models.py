...
# модеь продукта
class KuhniModel(models.Model):
    name = models.CharField(verbose_name='Название',max_length=120,blank=True, null=True ,unique=True)
    keyword = models.CharField(verbose_name='Ключи',max_length=120,blank=True, null=True ,unique=True)
    category = models.ForeignKey(Category, blank=True, null=True,related_name = 'category',on_delete=models.CASCADE,verbose_name='Категория',to_field='slug')
    podcateg = models.ForeignKey(Podcateg, blank=True, null=True,related_name = 'podcategory',on_delete=models.CASCADE,verbose_name='Подкатегория',to_field='slug')
...
def pre_save_product_slug(sender,instance, *args, **kwargs):
    if not instance.slug:
        slug = slugify(translit(instance.name, reversed=True))
        instance.slug = slug
pre_save.connect(pre_save_product_slug, sender=KuhniModel)
# -----------------------------end product-------------------------------------------------------------
#  --------------------------Gallery-------------------------------------------------------
# создание названия фотки
def image_gallary_folder(instance,filename):
    ...
    return "{0}/{1}".format(instance.slug,filename)
# фотки продукта
class KuhniImage(models.Model):
    product = models.ForeignKey(KuhniModel,blank=True, null=True, default=None,on_delete=models.CASCADE)
...
def pre_save_imagegallery_slug(sender,instance, *args, **kwargs):
    ...
    if not instance.slug:
        ...
        instance.slug = slug
pre_save.connect(pre_save_imagegallery_slug, sender=KuhniImage)
# ---------------------------------------end gallery----------------------------------
