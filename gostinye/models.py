...
# модеь продукта
class GostinyeModel(models.Model):
    name = models.CharField(verbose_name='Название',max_length=120,blank=True, null=True ,unique=True)
...
def image_gallary_folder(instance,filename):
    ...
    return "{0}/{1}".format(instance.slug,filename)
# фотки продукта
class GostinyeImage(models.Model):
    product = models.ForeignKey(GostinyeModel,blank=True, null=True, default=None,on_delete=models.CASCADE)
...
def pre_save_imagegallery_slug(sender,instance, *args, **kwargs):
    ...
    if not instance.slug:
        ...
        instance.slug = slug
pre_save.connect(pre_save_imagegallery_slug, sender=GostinyeImage)
# ---------------------------------------end gallery----------------------------------
